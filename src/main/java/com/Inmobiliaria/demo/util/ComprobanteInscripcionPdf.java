...
public class ComprobanteInscripcionPdf {
    ...
    public static byte[] generar(PagoInscripcionComprobante pagoInicial,
        ...
        try {
            ...
            String numComp = (pagoInicial.getComprobante() != null
            ...
            String montoEnLetras = NumeroALetras.convertir(
                    pagoInicial.getImportePagado(),
                    com.Inmobiliaria.demo.enums.Moneda.USD);

            String fechaPagoStr = pagoInicial.getFechaPago() != null
                    ? pagoInicial.getFechaPago().format(FMT) : "-";

            String medioPagoStr = pagoInicial.getMedioPago() != null
                    ? pagoInicial.getMedioPago().name() : "-";

            String numOp = (pagoInicial.getNumeroOperacion() != null
                    && !pagoInicial.getNumeroOperacion().isBlank())
                    ? "   N\u00b0 Op: " + pagoInicial.getNumeroOperacion() : "";

            String observaciones = (pagoInicial.getObservaciones() != null
                    && !pagoInicial.getObservaciones().isBlank())
            ...
            filaRecibo.addCell(new Cell()
                    ...
                    .setTextAlignment(TextAlignment.CENTER)
                    .setVerticalAlignment(VerticalAlignment.MIDDLE)
                    .add(new Paragraph(simbolo + " " + DF.format(pagoInicial.getImportePagado()))
    ...
    private static PdfFont cargarFuente(String path) throws Exception {
        byte[] bytes = StreamUtil.inputStreamToArray(
                ComprobanteInscripcionPdf.class.getClassLoader().getResourceAsStream(path));
